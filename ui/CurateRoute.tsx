...
function CandidateDetail({
  ...
  return <div className="flex max-h-[92vh] min-h-0 flex-col">
    ...
    <div className="min-h-0 overflow-y-auto p-5"><div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Meta label="Created" value={formatDate(candidate.created)} /><Meta label="Confidence" value={confidence === null ? '—' : `${Math.round(confidence * 100)}%`} /><Meta label="Approved" value={formatDate(candidate.approved_at)} /><Meta label="Quarantine until" value={formatDate(candidate.quarantine_until)} /></div><div className="mt-5"><p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-text-subtle">Full note</p><article className="max-h-[48vh] overflow-y-auto whitespace-pre-wrap rounded-xl border border-white/[0.08] bg-black/10 p-3 text-sm leading-6 text-text-muted">{candidate.body || 'No content available.'}</article></div>{candidate.sourceNotes?.length ? <div className="mt-5"><p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-text-subtle">Source notes</p><div className="space-y-3">{candidate.sourceNotes.map((note) => <section key={note.source} className="rounded-xl border border-white/[0.08] bg-black/10 p-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium text-text">{note.title}</p><span className={`text-[10px] uppercase tracking-[0.12em] ${note.found ? 'text-emerald-300' : 'text-text-subtle'}`}>{note.found ? 'found' : 'missing'}</span></div>{note.found && <p className="mt-2 max-h-40 overflow-y-auto whitespace-pre-wrap text-xs leading-5 text-text-muted">{note.body}</p>}</section>)}</div></div> : null}{tags.length > 0 && <DetailList label="Tags" items={tags} tone="sky" />}{sources.length > 0 && <DetailList label="Sources" items={sources} tone="neutral" />}{candidate.rejection_reason && <div className="mt-5 rounded-xl border border-rose-400/20 bg-rose-400/10 p-3"><p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-rose-300">Rejection feedback</p><p className="mt-2 text-sm leading-5 text-rose-100/80">{candidate.rejection_reason}</p></div>}</div>
